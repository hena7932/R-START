...
# 프레임 지우기                  
peple2 <- select(peple,-(기준년월), -(업종중분류코드), -(업종중분류코드드:업력구간), -(입사수:작업자명))
...
gifour <- peple5 %>% 
  ...
  filter(업종대분류 %in% c("C제조업", "G도매 및 소매업", "F건설업", "M전문; 과학 및 기술 서비스업", "N사업시설 관리; 사업 지원 및 임대 서비스업")) %>%
  group_by(시군구명, 업종대분류) %>%
  summarise(n=n())
...
graph +
  ...
  theme(legend.title = element_blank(),
        legend.text = element_text(face="bold", size=7, color="#330066"),
...
bottom10 %>%
  ggplot(aes(x = reorder(업종대분류, n), y = n)) +
  geom_bar(stat = "identity", position= 'dodge', width=.8, fill= "#F64F5C") +
  coord_flip() +# 가로형코드
  ylim(0, 45000) +
  theme_minimal() +
  theme(axis.text.x = element_text(vjust=0.4, size = 5, face = "bold"),
...
bottom10 %>%
  ...
  ylim(0, 55000) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle=65, vjust=0.6, size = 7, face = "bold"),
        axis.text.y = element_text(size =8, face = 'bold'),
        axis.title=element_text(size=6, face='bold'),
        title = element_text(size=10, face='bold'),
        legend.position = "top") +
